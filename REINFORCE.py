...
def demo_run(env, W, state_size, n_actions, norm_mean, norm_std):
    ...
    while not done:
        ...
        steps += 1
    return steps
def REINFORCE(train_env, demo_env, state_size, n_actions, n_iter, gamma, eta,
              demo_interval=300):
    ...
    for episode in range(n_iter):
        ...
        for t in range(T):
            ...
            dW += eta * G_t * grad_log
        W += dW
        returns_history.append(G[0])
        if (episode + 1) % demo_interval == 0:
            demo_steps = demo_run(demo_env, W, state_size, n_actions,
                                  norm_mean, norm_std)
            print(f"Iteration {episode + 1:4d} | Train: {T:3d} steps | "
                  f"Demo: {demo_steps:3d} steps | Return: {G[0]:6.2f}")
    return W, episode_steps, returns_history
if __name__ == "__main__":
    train_env = gym.make("CartPole-v1")
    demo_env = gym.make("CartPole-v1", render_mode="human")
    W, episode_steps, returns_history = REINFORCE(
        train_env, demo_env, state_size=4, n_actions=2, n_iter=200,
        gamma=0.99, eta=0.1)
    train_env.close()
    demo_env.close()
    ...
    if len(episode_steps) >= window:
        ...
        plt.xlabel('训练轮次')
        plt.ylabel('步数')
        plt.title('REINFORCE — 步数滑动平均')
        plt.grid(True, alpha=0.3)
        plt.show()
    plt.figure(figsize=(10, 5))
    plt.plot(returns_history, linewidth=0.5, color='#4CAF50')
    plt.xlabel('训练轮次')
    plt.ylabel('折扣回报')
    plt.title('REINFORCE — 初始状态折扣回报')
    plt.grid(True, alpha=0.3)
    plt.show()
